...
std::string
CV8File::GetEpfModule(const boost::filesystem::path &file)
{
	char *pFileData = NULL;

	int ret = 0;

    boost::filesystem::ifstream is(file, std::ios_base::binary);
    is.seekg(0, is.end);
    UINT FileDataSize = is.tellg();
    is.seekg(0, is.beg);

    pFileData = new char[FileDataSize];
    UINT readbytes = is.readsome(pFileData, FileDataSize);

    if (readbytes != FileDataSize) {
        // return "FILE_READ_ERROR";
        throw std::exception(); // TODO: Внятное исключение
    }

    CV8File epf;
	ret = epf.LoadFile(reinterpret_cast<BYTE*>(pFileData), FileDataSize);
	...
	for(UINT ElemNum = 0; ElemNum < epf.ElemsNum; ++ElemNum)
	{

        if (epf.pElems[ElemNum].IsV8File) {

            CV8File &child = epf.pElems[ElemNum].UnpackedData;
            for (UINT ElemNum = 0; ElemNum < child.ElemsNum; ++ElemNum) {
                child.GetElemName(child.pElems[ElemNum], ElemName, &ElemNameLen);
                if (strcmp(ElemName, "text") == 0) {

                    UINT L = child.pElems[ElemNum].DataSize;
                    char *mbuf = new char[L + 1];
                    strncpy(mbuf, reinterpret_cast<char*>(child.pElems[ElemNum].pData), L);
                    mbuf[L] = 0;
